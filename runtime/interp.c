...
/* Per-instruction facts the system-mode cycle count needs, set while the
   instruction executes (reading memory again afterwards could hit a
   register with read side effects). */
static int cross;       /* indexed address crossed a page */
static int taken;       /* branch taken */
static long moved;      /* MVN/MVP bytes */
static int waiting;     /* WAI executed, no interrupt yet */
...
static EA ea(CPU *c, enum mode md)
{
    ...
    case M_DPIY: {
        uint32_t b = db | rd_bank0_16((uint16_t)(c->DP + fetch8(c)));
        cross = ((b ^ (b + c->Y)) & 0xFF00) != 0;
        return full(b + c->Y);
    }
    ...
    case M_ABSX: {
        uint32_t b = db | fetch16(c);
        cross = ((b ^ (b + c->X)) & 0xFF00) != 0;
        return full(b + c->X);
    }
    case M_ABSY: {
        uint32_t b = db | fetch16(c);
...
/* ---- system mode: cycles, emulation mode, interrupts ---- */
...
static unsigned master_per_cycle(uint8_t pb, uint16_t pc)
{
    int rom = pb >= 0xC0 || ((pb & 0x7F) < 0x40 && pc >= 0x8000);
    return (pb & 0x80) && rom && bus_fastrom() ? 6 : 8;
}

/* Opcodes whose emulation-mode behavior matches what is implemented: flag
   ops, XCE, REP/SEP (M/X stay forced to 1), immediate loads, NOP, jumps,
   branches. Enough for the reset stub. Everything else in emulation mode
   (page-wrapping direct page, 6502 stack rules for 65816 pushes, BRK/IRQ
   frames) is not implemented and fails loudly. */
static int emulation_ok(uint8_t op)
{
    switch (op) {
    case 0x18: case 0x38: case 0x58: case 0x78: case 0xD8: case 0xF8: case 0xB8:
    case 0xFB: case 0xEA: case 0xC2: case 0xE2: case 0xA9: case 0xA2: case 0xA0:
    case 0x4C: case 0x5C:
...
unsigned interp_step(CPU *c)
{
    ...
    interp_budget = 0;
    if (waiting)
        ct_fatal("interp: step while waiting for an interrupt (WAI)");
    uint32_t at = (uint32_t)c->PB << 16 | c->PC;
    if (ct_trace_hook)
        ct_trace_hook(c, at);
    int m16 = !c->m, x16 = !c->x, dl = (c->DP & 0xFF) != 0, native = !c->e;
    unsigned speed = master_per_cycle(c->PB, c->PC);
    uint8_t op = fetch8(c);
    ...
    if ((pen & P_RTI) && native)
        n += 1;
    if (!n)
        ct_fatal("interp $%06X: no cycle count for opcode $%02X", at, op);
    return n * speed;
}
