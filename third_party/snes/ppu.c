...
void ppu_write(Ppu* ppu, uint8_t adr, uint8_t val) {
  switch(adr) {
    ...
    case 0x01: {  // OBSEL
      // ct-recomp: upstream hardcoded objSize/objTileAdr1/2 in ppu_reset()
      // for ALTTP's one fixed OBSEL value and asserted no other value was
      // ever written. Decode it for real; formula reproduces the old
      // defaults exactly for val == 2 (size 0, base $4000/$5000).
      ppu->objSize = (val >> 5) & 7;
      ppu->objTileAdr1 = (uint16_t)(val & 7) << 13;
      ppu->objTileAdr2 = (uint16_t)(ppu->objTileAdr1 + (((val >> 3) & 3) + 1) * 0x1000);
      break;
    }
    case 0x02: {
    ...
    case 0x03: {
      // ct-recomp: bit 7 (OAM priority rotation) is accepted but has no
      // effect -- upstream never stored it either. Flagged as a gap.
    ...
    case 0x05: {  // BGMODE
      // ct-recomp: upstream asserted mode 1/7 only (all ALTTP ever used).
      // Chrono Trigger uses other modes; bitDepthsPerMode/layersPerMode
      // below are already indexed for all 8, but only 1/7 have been run
      // against a real game -- unverified until there's a frontend to
    ...
    case 0x15: {  // VMAIN
      ...
      // ct-recomp: bits 2-3 (VRAM address remapping) accepted but not
      // implemented -- VMDATAL/H below always write linearly. Gap.
    ...
    case 0x1a: {  // M7SEL
      // ct-recomp: upstream asserted ALTTP's one fixed value; all four
      // bits below are already decoded correctly for any value.
    ...
    case 0x2a:  // WBGLOG
    case 0x2b:  // WOBJLOG
      // ct-recomp: window logic (OR/AND/XOR/XNOR between window 1/2) is
      // hardcoded to OR throughout PpuWindows_Calc; upstream asserted OR
      // was the only value ever written. Not implemented. Gap.
      break;
    case 0x2c:  // TM
    ...
    case 0x2f:  // TSW
      ...
      break;
    case 0x30: {  // CGWSEL
      // ct-recomp: bit 0 (direct color mode, mode 3/4/7 8bpp BGs) accepted
      // but not implemented -- upstream asserted it was always zero. Gap.
    ...
    case 0x33: {
      // ct-recomp: SETINI. Upstream asserted val == 0 (ALTTP never set
      // any bit) and only ever decoded bit 6 (extbg). Bits 0/1 (interlace),
      // 2 (overscan/240 lines) and 3 (pseudo-hires) are accepted but have
      // no effect anywhere in the renderer -- there's no field-alternation
      // or 512-wide output path at all. Real gap, not just untested.
